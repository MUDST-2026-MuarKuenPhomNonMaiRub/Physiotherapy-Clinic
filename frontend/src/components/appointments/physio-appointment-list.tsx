...
export function PhysioAppointmentList({ list, showDate }: { list: Appointment[]; showDate: boolean }) {
  ...
  async function begin(appointmentId: string, onDone?: () => void) {
    try {
      await startService(appointmentId);
      onDone?.();
      toast.success("Service started");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Could not start this service");
    }
  }
            ...
            <>
              ...
              <SheetFooter className="flex-row border-t border-border">
                ...
                {selected.status === "ARRIVED" ? (
                  <Button
                    className="flex-1"
                    onClick={() => void begin(selected.id, () => setSelectedId(null))}
                  >
                    Start Service
                  </Button>
                ) : (
