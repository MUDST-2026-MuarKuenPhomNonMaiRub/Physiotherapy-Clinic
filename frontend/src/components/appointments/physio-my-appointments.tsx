...
const todayLong = new Date(`${TODAY}T00:00:00`).toLocaleDateString("en-GB", {
  weekday: "long",
  day: "numeric",
  month: "long",
  year: "numeric",
...
export function PhysioMyAppointments({ staffId }: { staffId: string }) {
  ...
  const today = mine.filter((a) => a.date === TODAY).sort((a, b) => a.startTime.localeCompare(b.startTime));
  const upcoming = mine
    .filter((a) => a.date > TODAY && ["CONFIRMED", "ARRIVED", "IN_SERVICE", "RESCHEDULED"].includes(a.status))
  ...
  const remainingToday = today.filter((a) => ["CONFIRMED", "ARRIVED", "IN_SERVICE"].includes(a.status)).length;
  const nextPatient = today.find((a) => ["CONFIRMED", "ARRIVED", "IN_SERVICE"].includes(a.status));
    ...
    <>
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {today.length} appointment{today.length === 1 ? "" : "s"} today · {remainingToday} remaining
            </p>
          </div>
        </div>
      ...
      <Tabs defaultValue="today">
        ...
        <TabsContent value="today">
          <PhysioAppointmentList list={today} showDate={false} />
        </TabsContent>
        <TabsContent value="month">
          <PhysioMonthCalendar appointments={mine} patients={patients} services={services} today={TODAY} />
        </TabsContent>
