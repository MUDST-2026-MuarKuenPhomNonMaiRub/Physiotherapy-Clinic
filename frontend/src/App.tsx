...
function read() {
  const [role, path] = location.hash.slice(1).split('/');
  return { role: (role as Role) || null, path: path || '' };
}
export default function App() {
  const initial = read();
  const [role, setRole] = useState<Role | null>(initial.role);
  const [path, setPath] = useState(
    initial.path || (initial.role ? defaults[initial.role] : 'branches'),
  );
  useEffect(() => {
    const fn = () => {
      const v = read();
      setRole(v.role);
      if (v.role) setPath(v.path || defaults[v.role]);
    };
    ...
    <div className="reference-login">
      <section className="login-showcase">
        <div className="login-logo">
          <span>⌁</span>PhysioCare Clinic ERP
        </div>
        <div>
          <h1>Run your clinic&apos;s entire workflow, end to end.</h1>
          <p>
            An internal system for clinic staff — registration, appointments, treatment, checkout,
            course balances and reporting, all handled by your team on behalf of every patient.
          </p>
          <div className="feature-grid">
            <span>▣ Appointments & Visits</span>
            <span>▤ Course Balance Tracking</span>
            <span>▦ Multi-Branch Access</span>
            <span>% Commission & Reports</span>
          </div>
        </div>
        <small>© 2026 PhysioCare Clinic. Interactive product mock-up — no real data.</small>
      </section>
      <section className="login-access">
        <div className="login-form">
          ...
          {accounts.map((a) => (
            <button className="demo-account" key={a.role} onClick={() => onEnter(a.role)}>
              <span className="demo-icon">{a.title.slice(0, 2)}</span>
              <span>
                <b>{a.title}</b>
