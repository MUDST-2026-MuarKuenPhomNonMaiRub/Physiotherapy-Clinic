...
export default function MasterDataSettingsPage() {
  ...
  async function save() {
    ...
    try {
      if (editing) { await updateMasterDataItem(editing.id, { value }); toast.success("Updated"); }
      else { await addMasterDataItem({ category: activeCategory, value, status: "ACTIVE" }); toast.success("Added"); }
      setOpen(false);
    } catch (error) {
