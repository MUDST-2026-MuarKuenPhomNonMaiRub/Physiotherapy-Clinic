...
export default function CommissionSettingsPage() {
  ...
  function save() {
    ...
    if (editing) { updateCommissionRule(editing.id, payload); toast.success("Commission rule updated"); }
    else { addCommissionRule({ ...payload, status: "ACTIVE" }); toast.success("Commission rule created"); }
    setOpen(false);
  }
                  ...
                  <TableRow key={r.id} className="[&>td]:py-3.5">
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <Switch checked={r.status === "ACTIVE"} onCheckedChange={() => toggleCommissionRuleStatus(r.id)} />
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
