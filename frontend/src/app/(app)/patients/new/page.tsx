...
export default function NewPatientPage() {
  ...
  const hnPreview = useMemo(() => {
    if (!form.registrationBranchId) return null;
    const branchIndex = branches.findIndex((b) => b.id === form.registrationBranchId);
    const branchSeqNo = branchIndex >= 0 ? branchIndex + 1 : 9;
    const now = new Date(`${TODAY}T00:00:00`);
    const year = now.getFullYear();
    const month = now.getMonth() + 1;
    const prefix = `${String(year % 100).padStart(2, "0")}${String(branchSeqNo).padStart(2, "0")}${String(month).padStart(2, "0")}`;
    const sameMonth = patients.filter((p) => p.registrationBranchId === form.registrationBranchId && p.hn.slice(0, 6) === prefix);
    return generateHN(branchSeqNo, year, month, sameMonth.length + 1);
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="mx-auto max-w-4xl space-y-5 pb-10">
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button type="submit">Save & Generate HN</Button>
        </div>
      </form>
    </>
  );
