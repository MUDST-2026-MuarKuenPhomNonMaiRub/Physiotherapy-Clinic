...
export default function AppointmentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  function doReschedule() {
    ...
    const created = rescheduleAppointment(apt!.id, newDate, newStart, newEnd, rescheduleReason.trim() || undefined);
  ...
  return (
    <>
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            ...
            <div className="flex flex-col gap-2">
              {apt.status === "CONFIRMED" && (
                <>
                  {canFrontDeskOps && (
                    <Button onClick={() => { checkInAppointment(apt.id); toast.success("Patient checked in"); }}>
              ...
              {apt.status === "ARRIVED" && (
                canOperate ? (
                  <Button onClick={() => { startService(apt.id); toast.success("Service started"); }}>
              ...
              {apt.status === "IN_SERVICE" && (
                canOperate ? (
                  <Button onClick={() => { completeService(apt.id); toast.success("Service completed"); }}>
      ...
      <ConfirmDialog
        ...
        destructive
        onConfirm={() => { markNoShow(apt.id); setNoShowOpen(false); toast.success("Marked as No Show"); }}
      />
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => { cancelAppointment(apt.id, cancelReason.trim()); setCancelOpen(false); toast.success("Appointment cancelled"); }}
            >
              Cancel Appointment
            </Button>
          </DialogFooter>
