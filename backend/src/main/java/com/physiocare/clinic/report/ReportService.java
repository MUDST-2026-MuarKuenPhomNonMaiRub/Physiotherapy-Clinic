  ...
  public Object commissions(
    ...
    return db.queryForList(
        "SELECT employee_id treating_employee_id,sum(treatment_fee) treatment_fee,"
            ...
            // Scoped by the visit's branch when the usage came through an
            // appointment; a checkout-only usage has no visit row, so it falls
            // back to the branch the course was sold at rather than vanishing.
            + "ca.gross_commission_allocation FROM commission_allocations ca "
            + "JOIN patient_courses pc ON pc.id=ca.patient_course_id "
            + "LEFT JOIN visits v ON v.id=ca.visit_id WHERE ca.allocation_status='ALLOCATED' AND "
            + "ca.visit_date BETWEEN ? AND ? AND COALESCE(v.branch_id,pc.branch_id)=? UNION ALL "
            + "SELECT ca.treating_employee_id employee_id,ca.treatment_fee_amount treatment_fee,"
            + "0 owner_net,0 gross_commission_allocation FROM commission_allocations ca "
            + "JOIN patient_courses pc ON pc.id=ca.patient_course_id "
            + "LEFT JOIN visits v ON v.id=ca.visit_id WHERE ca.allocation_status='ALLOCATED' AND "
            + "ca.visit_date BETWEEN ? AND ? AND COALESCE(v.branch_id,pc.branch_id)=? "
            + "AND ca.treating_employee_id<>ca.case_owner_employee_id"
